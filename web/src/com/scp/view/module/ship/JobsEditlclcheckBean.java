...
@ManagedBean(name = "pages.module.ship.jobseditlclcheckBean", scope = ManagedBeanScope.REQUEST)
public class JobsEditlclcheckBean extends MastDtlView {
...
//			disableAllButton(selectedRowData.getIsubmit());
	...
	public void showShipping() {
		if (this.mPkVal == -1l) {
			...
			shippingIframe.load("../ship/busshippinglclcheck.xhtml?id=" + this.mPkVal);
		}
	}
	...
	public GridDataProvider getJobnosGridDataProvider() {
		return this.jobsnosChooseService.getJobsnosDataProvider("AND jobtype = 'B'");
	}
...
//	}
	...
	public void createwmsin(){
		if(this.mPkVal ==-1l){
			...
			return;
		}else if(this.selectedRowData.getIscheck()==false){
			MessageUtils.alert("工作单未审核");
			return;
		}
		try {
			serviceContext.jobsMgrService.createwmsin(this.mPkVal,AppUtils.getUserSession().getUsercode());
	...
	public void createJobs() {
		try {
			serviceContext.jobsMgrService.createJobs(this.mPkVal , AppUtils.getUserSession().getUsercode());
			MessageUtils.alert("OK!");
		} catch (Exception e) {
			e.printStackTrace();
			MessageUtils.showException(e);
	...
	public void ischeckAjaxSubmit() {
		...
		String sql="";
		if (isCheck) {
			sql = "UPDATE fina_jobs SET ischeck = TRUE ,checktime = NOW(),checkter = '"+updater+"',updater = '" + updater + "',updatetime=NOW() WHERE id ="+this.mPkVal+";";
		}else {
			sql = "UPDATE fina_jobs SET ischeck = FALSE ,checktime = NULL,checkter = NULL,updater = '" + updater + "',updatetime=NOW() WHERE id ="+this.mPkVal+";";
		}
		try {
