...
@ManagedBean(name = "pages.module.ship.jobseditpodBean", scope = ManagedBeanScope.REQUEST)
public class JobsEditpodBean extends MastDtlView {
...
//		}
	...
	public void showShipping() {
		if (this.mPkVal == -1l) {
			...
			shippingIframe.load("../ship/busshippingpod.xhtml?id=" + this.mPkVal);
		}
	}
...
//	}
	...
	public void showConstruct() {
		if (this.mPkVal == -1l) {
			...
			constructIframe.load("../ship/jobschildpod.xhtml?id=" + this.mPkVal);
		}
	}
...
//		}
	...
	public void addMaster() {
		...
		selectedRowData.setJobtype("I");
		selectedRowData.setLdtype("F");
		selectedRowData.setIsclc(true);
		//执行客户端脚本
...
//	}
	 ...
	 */
	public void setFnos(){
		String sql = "SELECT f_lists((SELECT nos FROM fina_jobs WHERE id = k.jobidfm AND isdelete = FALSE)) AS fnos FROM fina_jobs_link k  WHERE k.jobidto = "+this.mPkVal;
	 ...
	 * 加载海外单号
	 */
	public void setNextnos() {
		String sql = "SELECT string_agg((SELECT nos FROM fina_jobs WHERE id = k.jobidto AND isdelete = FALSE),',') AS nextnos " 
		...
		try {
			Map m = serviceContext.jobsMgrService.daoIbatisTemplate
					.queryWithUserDefineSql4OnwRow(sql);
			this.nextnos = (String) m.get("nextnos");
		} catch (NoRowException e) {
			this.nextnos = "";
		}

		this.update.markUpdate(UpdateLevel.Data, "nextnos");
	}
}
