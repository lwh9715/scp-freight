...
@ManagedBean(name = "pages.module.ship.shipbillcopychooseBean", scope = ManagedBeanScope.REQUEST)
public class ShipBillCopyChooseBean extends GridView {
	...
	public Long getshipid(Long id){
		String sql="SELECT id FROM bus_shipping where isdelete =false AND jobid= "+id;
		Map m=serviceContext.daoIbatisTemplate.queryWithUserDefineSql4OnwRow(sql);
		return (Long)m.get("id");
		
	}
	/**
	 *获取当前当前工作单号的pod,customerabbr 
	 */
