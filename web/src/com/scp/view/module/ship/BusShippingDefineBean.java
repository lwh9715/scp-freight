...
@ManagedBean(name = "pages.module.ship.busshippingdefineBean", scope = ManagedBeanScope.REQUEST)
public class BusShippingDefineBean extends FormView {
	...
	@Override
	public void refresh() {
		selectedRowData = this.serviceContext.busShippingMgrService.busShippingDao.findById(Long.parseLong(billid));
		try {
		...
		mblreport = ConfigUtils.findSysCfgVal("mbl_report");
...
//	}
	...
	public void savelastbill(){
		try {
			...
			String upg = "UPDATE bus_shipping set hblrpt = '" + hblrptjs + "' where id = " + billid + ";";
			this.serviceContext.userMgrService.sysUserDao.executeSQL(upg);
		} catch (Exception e) {
			MessageUtils.showException(e);
		}
	...
	public void savelastbillmbl(){
		try {
			...
			String upg = "UPDATE bus_shipping set mblrpt = '" + mblrptjs + "' where id = " + billid + ";";
			this.serviceContext.userMgrService.sysUserDao.executeSQL(upg);
		} catch (Exception e) {
			MessageUtils.showException(e);
		}
	...
	public void printMbl() {
		...
		String openUrl = rpturl + "/reportJsp/showReport.jsp?raq=/define/"
				+ mblRpt;
		String upg = "UPDATE bus_shipping set mblrpt = '" + mblRpt + "' where id = " + billid + ";";
		this.serviceContext.userMgrService.sysUserDao.executeSQL(upg);
	...
	public void printOrder() {
		...
		Map map = null ;
		try {
			String querySql = "SELECT 1 FROM sys_report where modcode = 'shippingorder' AND templete IS NOT NULL AND filename = '"+orderRpt+"';";
		...
		if("Y".equals(bill_single_new) && (map != null && map.size() == 1)){
			String url = rpturl + "/reportEdit/file/print.jsp?rp="+orderRpt+"&b="+this.billid+"&u="+AppUtils.getUserSession().getUserid()+"&reporttype=shippingorder";
	...
	private String getArgsusr() {
		BusShipping bs = serviceContext.busShippingMgrService.busShippingDao.findById(Long.parseLong(billid));
	...
	public void printBook() {
		...
		String openUrl = rpturl + "/reportJsp/showReport.jsp?raq=/define/"
				+ bookRpt;
		String upg = "UPDATE bus_shipping set bookrpt = '" + bookRpt + "' where id = " + billid + ";";
		this.serviceContext.userMgrService.sysUserDao.executeSQL(upg);
	...
	public List<SelectItem> getMblrpt() {
		try {
			if("demo".equalsIgnoreCase(AppUtils.getUserSession().getUsercode())){
				return CommonComBoxBean.getComboxItems("d.filename", "d.code ",
						"sys_report AS d", "WHERE modcode='shippingmbl' AND isdelete = FALSE ",
				"order by code");
			}else{
				String sql = "WHERE modcode='shippingmbl' AND isdelete = FALSE AND (userid IS NULL OR ispublic OR userid = "+AppUtils.getUserSession().getUserid()+")";
				String qry = "\nAND (d.corpid iS NULL OR (EXISTS(SELECT 1 FROM sys_user_corplink x WHERE x.corpid = d.corpid AND x.ischoose = TRUE AND userid ="+AppUtils.getUserSession().getUserid()+")))";
				if(((ApplicationConf) AppUtils.getBeanFromSpringIoc("applicationConf")).isSaas())sql += qry;//非saas模式不控制
				return CommonComBoxBean.getComboxItems("d.filename", "d.code ",
						"sys_report AS d", sql,
	...
	public List<SelectItem> getOrderrpt() {
		try {
			String sql = "WHERE modcode='shippingorder' AND isdelete = FALSE";
			String qry = "\nAND (d.corpid iS NULL OR (EXISTS(SELECT 1 FROM sys_user_corplink x WHERE x.corpid = d.corpid AND x.ischoose = TRUE AND userid ="+AppUtils.getUserSession().getUserid()+")))";
			if(((ApplicationConf) AppUtils.getBeanFromSpringIoc("applicationConf")).isSaas())sql += qry;//非saas模式不控制
			return CommonComBoxBean.getComboxItems("d.filename", "d.code ",
					"sys_report AS d", sql,
	...
	public List<SelectItem> getBookrpt() {
		try {
			String sql = "WHERE modcode='shippingbook' AND isdelete = FALSE";
			String qry = "\nAND (d.corpid iS NULL OR (EXISTS(SELECT 1 FROM sys_user_corplink x WHERE x.corpid = d.corpid AND x.ischoose = TRUE AND userid ="+AppUtils.getUserSession().getUserid()+")))";
			if(((ApplicationConf) AppUtils.getBeanFromSpringIoc("applicationConf")).isSaas())sql += qry;//非saas模式不控制
			return CommonComBoxBean.getComboxItems("d.filename", "d.code ",
					"sys_report AS d", sql,
...
//		super.save();
	...
	public void onlineFeedLink(){
		...
		try {
			...
			//System.out.println(apiKeyGenerate(currentTimeMillis+"","SZPE19110004"));
		...
		String url = csUrlbase+"/si/esi/"+this.selectedRowData.getHblno()+"?tp="+currentTimeMillis+"&key="+ApiKey+"&billtype="+h;
		...
		if(csUrlbase.indexOf("so")>1){
			...
			url = csUrlbase+"/esi?method=esiInfo&nos="+this.selectedRowData.getHblno()+"&tp="+currentTimeMillis+"&key="+ApiKey+"&key2="+jobid+"&billtype="+h;
		}
	...
	public void onlineFeedLinkmbl(){
		...
		String jobid = String.valueOf(this.selectedRowData.getJobid());
		String url = csUrlbase+"/esi?method=esiInfo&nos="+this.selectedRowData.getMblno()+"&tp="+currentTimeMillis+"&key="+ApiKey+"&key2="+jobid+"&billtype="+m;
		sb.append(url);
		SimpleDateFormat dateformat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		String time = dateformat.format(currentTimeMillis+parseInt * 60 * 60 * 1000).toString();
		sb.append("\n\n链接有效时间至："+time) ;
	...
	public static String apiKeyGenerate(String ApiSecret, String ApiSalt) throws UnsupportedEncodingException{
	...
	public void showTipsCount(){
		try{
			String messageTipsql = "SELECT COUNT(1) FROM sys_snapshots where linkid = "+billid;
			Map m = serviceContext.daoIbatisTemplate.queryWithUserDefineSql4OnwRow(messageTipsql);
			if(m!=null&&m.get("count")!=null){
				Browser.execClientScript("showButtonTips('esihistorybutton',"+m.get("count")+")");
			}
		}catch(Exception e){
			e.printStackTrace();
		}
