...
@ManagedBean(name = "pages.module.ship.busshippingpodBean", scope = ManagedBeanScope.REQUEST)
public class BusShippingpodBean extends MastDtlView {
	...
	public void doServiceSaveMaster() {
		if(this.selectedRowData.getPolid()==null){
			MessageUtils.alert("起运港为空!请下拉选择");
			return;
		}
		
		if(this.selectedRowData.getPddid()==null){
			MessageUtils.alert("卸货港为空!请下拉选择");
			return;
		}
		...
		try {
			...
			if (cs.size() == 1) {
				if("0".equals(porttype)){
					...
					this.selectedRowData.setPol((String)cs.get(0).get("code"));
					...
					this.selectedRowData.setPod((String)cs.get(0).get("code"));
					...
					this.selectedRowData.setPdd((String)cs.get(0).get("code"));
	...
	public void portGrid_ondblclick() {
		...
		if("0".equals(porttype)){
			...
			this.selectedRowData.setPol((String) m.get("code"));
			this.update.markUpdate(UpdateLevel.Data, "polid");
			...
			this.selectedRowData.setPod((String) m.get("code"));
			this.update.markUpdate(UpdateLevel.Data, "podid");
			...
			this.selectedRowData.setPdd((String) m.get("code"));
