...
@ManagedBean(name = "pages.module.carmgr.caroildtlBean", scope = ManagedBeanScope.REQUEST)
public class CarOilDtlBean extends MastDtlView {
	...
	public void showArapEdit() {
		try {
			if (this.mPkVal == -1l) {
				...
				arapIframe.load("../finance/arapedit.faces?id=" + this.mPkVal
						+ "&jobid=" + findJobId());
			}
	...
	@Override
	public void init() {
		selectedRowData = new CarOliRecord();
	...
	public void refreshMasterForm() {
		this.selectedRowData = serviceContext.carOilRecordMgrService.carOliRecordDao
	...
	public void ischeckAjaxSubmit() {
		...
		if (isCheck) {
			sql = "UPDATE car_oli_record SET ischeck = TRUE ,checktime = '"
					+ date + "',checkter = '" + updater + "' WHERE id ="
					+ this.mPkVal + ";";
		} else {
			sql = "UPDATE car_oli_record SET ischeck = FALSE ,checktime = NULL,checkter = NULL WHERE id ="
					+ this.mPkVal + ";";
		}
		try {
	...
	public void addMaster() {
		...
		this.selectedRowData = new CarOliRecord();
		this.mPkVal = -1l;
	...
	public void delMaster() {
		if (selectedRowData.getIscheck()) {
			...
			serviceContext.carOilRecordMgrService.removeDate(this.mPkVal);
			this.addMaster();
			this.alert("OK");
