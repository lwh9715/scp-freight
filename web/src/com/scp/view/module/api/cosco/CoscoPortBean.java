...
@ManagedBean(name = "pages.module.api.cosco.coscoportBean", scope = ManagedBeanScope.REQUEST)
public class CoscoPortBean extends GridFormView{
	...
	public Map getQryClauseWhere(Map<String, Object> queryMap) {
		...
		if(!StrUtils.isNull(qryCode)){
			qry += "\nAND (name LIKE '%"+qryCode+"%' OR code LIKE '%"+qryCode+"%' OR portfullnamecn LIKE '%"+qryCode+"%' OR portfullnameen LIKE '%"+qryCode+"%' OR ctryregionname LIKE '%"+qryCode+"%' OR cityname LIKE '%"+qryCode+"%')";
		}
	...
	public void deleter(){
		...
		if (ids == null || ids.length == 0) {
			...
			for (String id : ids) {
				stringBuilder.append("\nUPDATE api_cosco_port set isdelete = TRUE WHERE id = " + id + ";");
			}
