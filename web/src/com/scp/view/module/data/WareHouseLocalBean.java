...
@ManagedBean(name = "pages.module.data.warehouselocalBean", scope = ManagedBeanScope.REQUEST)
public class WareHouseLocalBean extends DynamicGridView{
	...
	private void changeWareHouse() {
		...
		if (warehouseid != null && warehouseid.length() > 0) {
			try {
				wareHouseAreaComboItems = CommonComBoxBean.getComboxItems("d.id",
						"d.code ",
		...
		try {
			...
			for (int i = 0; i < rows; i++) {
				String qrysql = "\nSELECT "
					+ "\n l.id "
					+ "\n,code "
					+ "\n,(EXISTS(SELECT 1 FROM _wms_stock x WHERE x.locid = l.id AND x.piece > 0) OR EXISTS(SELECT 1 FROM wms_in_locdtl x , wms_indtl y , wms_in z , _wms_stock zz WHERE x.locid = l.id AND x.pieces > 0 AND x.indtlid = y.id AND x.iscompleted = false AND y.inid = z.id AND z.ischeck = TRUE AND z.isdelete = FALSE AND y.isdelete = false AND zz.indtlid = y.id AND zz.piece > 0)) AS isempty" 
	...
	public void showLocalDtlAction() {
		...
		dtlIFrame.setSrc("./warehouselocaldtl.faces?code=" + code+"&areaid="+areaid);
	...
	private void dtlEditDialogClose() {
		String blankUrl = AppUtils.getContextPath() + "/pages/common/blank.html";
