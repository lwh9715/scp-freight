...
@ManagedBean(name = "pages.module.data.warehouseBean", scope = ManagedBeanScope.REQUEST)
public class WareHouseBean extends GridFormView {
	...
	public void grid_ondblclick() {
		...
		String url = "./warehousearea.xhtml?id="+this.getGridSelectId();
		int width = 1024;
		int height = 768;
	...
	public void userAssign() {
		String winId = "_userassign";
		if(this.getGridSelectId() == -1) {
			MessageUtils.alert("Please choose one!");
		} else {
			String url = "./userassign.xhtml?linkid="+this.getGridSelectId();
			AppUtils.openWindow(winId, url);
		}
	}
	...
	public void importDataBatch() {
		if (StrUtils.isNull(importDataText)) {
			...
			try {
				String callFunction = "f_imp_warehouse";
