...
@ManagedBean(name = "pages.module.crm.clientmgreditBean", scope = ManagedBeanScope.REQUEST)
public class ClientmgreditBean extends MastDtlView {
...
//				arapIframe.load(blankUrl);
				arapIframe.load( "../customer/assigneduser.xhtml?linktype=C&id="+this.mPkVal);
			}
		} catch (Exception e) {
	...
	public void showCorpinv() {
		try {
			if (this.mPkVal == -1l) {
				...
				corpinvIframe.load( "../customer/syscorpinv.xhtml?id="+this.mPkVal);
			}
		} catch (Exception e) {
			MessageUtils.showException(e);
		}
