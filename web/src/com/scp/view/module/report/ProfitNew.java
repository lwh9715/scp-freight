...
@WebServlet("/profitnew")

@ManagedBean(name = "pages.module.report.profitNewBean")
public class ProfitNew extends BaseServlet {
    ...
    public BaseView BusCommerceJobId(HttpServletRequest request) {
        ...
        try {
            ...
            String Sfield = "";
            String Ofield = "";
            String corpidStr = " AND a.corpid = " + session.get("corpid").toString();
            ...
            for (int i = 0; i < columndefsJSONArray.size(); i++) {
                ...
                if (thisJSONObject.containsValue("custype") && !thisJSONObject.containsKey("hide")) {
                    Afield += "\t\t\t\t\t,(SELECT namec from dat_filedata WHERE isleaf = TRUE AND isdelete = false AND fkcode = 40 AND code =(SELECT custype  FROM bus_customs WHERE jobid = t.id AND clstype='O' " +
                            "AND isdelete = FALSE order by nos LIMIT 1)) AS custype\n";
                }
                if (thisJSONObject.containsValue("status") && !thisJSONObject.containsKey("hide")) {
                    Afield += "\t\t\t\t\t,(SELECT namec from dat_filedata WHERE isleaf = TRUE AND isdelete = false AND fkcode = 100 AND code =(SELECT status  FROM bus_customs WHERE jobid = t.id AND clstype='O' " +
                            "AND isdelete = FALSE order by nos LIMIT 1)) AS status\n";
                }
                ...
                if (thisJSONObject.containsValue("cntreleasetime") && !thisJSONObject.containsKey("hide")) {
                    Afield += "\t\t\t\t\t,to_char( (SELECT cntrelease_time  FROM bus_customs WHERE jobid = t.id AND isdelete = FALSE order by nos LIMIT 1) ::timestamp, 'yyyy-mm-dd hh24:mi:ss') AS " +
                            "cntreleasetime\n";
                }
                ...
                if (thisJSONObject.containsValue("truckstate") && !thisJSONObject.containsKey("hide")) {
                    Afield += "\t\t\t\t\t,(SELECT (CASE WHEN x.truckstate = 'I' THEN '初始' ELSE '完成' END) FROM bus_truck x,dat_filedata y WHERE x.isdelete = FALSE AND x.jobid = t.id AND x.truckstate = y.code " +
                            "LIMIT 1) AS truckstate\n";
                }
                ...
                if (thisJSONObject.containsValue("trucktype") && !thisJSONObject.containsKey("hide")) {
                    Afield += "\t\t\t\t\t,(SELECT y.namec FROM bus_truck x,dat_filedata y WHERE x.isdelete = FALSE AND x.jobid = t.id AND x.trucktype = y.code AND x.trucktype IS NOT NULL AND x.trucktype <> '' " +
                            "AND y.isleaf = TRUE AND y.isdelete = false AND y.fkcode = 110 LIMIT 1) AS trucktype\n";
                }
                ...
                if (thisJSONObject.containsValue("finance") && !thisJSONObject.containsKey("hide")) {
                    Afield += "\t\t\t\t\t,(SELECT string_agg(x.namec,',') FROM sys_user x , sys_user_assign y WHERE x.id =y.userid AND y.roletype = 'F' AND y.linkid = (select id from bus_shipping bs where bs" +
                            ".isdelete = false and bs.jobid=t.id limit 1) AND  COALESCE(y.isdelete,false) = false)  AS finance\n";
                }
                if (thisJSONObject.containsValue("mbltypedesc") && !thisJSONObject.containsKey("hide")) {
                    Afield += "\t\t\t\t\t,(SELECT namec FROM dat_filedata WHERE isdelete = false AND fkcode = 150 AND isleaf = TRUE AND code = (select mbltype from bus_shipping bs where bs.isdelete = false and " +
                            "bs.jobid=t.id limit 1) LIMIT 1) AS mbltypedesc\n";
                }
                ...
                if (thisJSONObject.containsValue("clearstate") && !thisJSONObject.containsKey("hide")) {
                    Afield += "\t\t\t\t\t,(SELECT namec from dat_filedata WHERE isleaf = TRUE AND isdelete = false AND fkcode = 120 AND code =(SELECT clearingstate  FROM bus_customs WHERE jobid = t.id AND " +
                            "clstype='I' AND isdelete = FALSE order by nos LIMIT 1)) AS clearstate\n";
                }
                ...
                if (thisJSONObject.containsValue("bookinger") && !thisJSONObject.containsKey("hide")) {
                    Afield += "\t\t\t\t\t,(SELECT string_agg(COALESCE(namec,'')||'/'||COALESCE(namee,''),',') FROM sys_user x , sys_user_assign y WHERE x.id =y.userid AND y.roletype = 'T' AND y.linkid = (select" +
                            " id from bus_shipping bs where bs.isdelete = false and bs.jobid=t.id limit 1)  AND  COALESCE(y.isdelete,false) = false)  AS bookinger\n";
                }
                if (thisJSONObject.containsValue("docnametime") && !thisJSONObject.containsKey("hide")) {
                    Afield += "\t\t\t\t\t,(SELECT y.inputtime FROM sys_user_assign y WHERE y.linktype = 'J' AND y.roletype = 'D' AND y.linkid = (select id from bus_shipping bs where bs.isdelete = false and bs" +
                            ".jobid=t.id limit 1)  AND COALESCE(y.isdelete,false) = false limit 1)  AS docnametime\n";
                }
                if (thisJSONObject.containsValue("sonoss") && !thisJSONObject.containsKey("hide")) {
                    Afield += "\t\t\t\t\t,(SELECT string_agg((c.sono)::text, ','::text) AS string_agg FROM bus_ship_container c WHERE (((((c.jobid = t.id) AND (c.isdelete = false)) AND (c.cntno IS NOT NULL)) " +
                            "AND ((c.cntno)::text <> ''::text)) AND (c.parentid IS NULL))) AS sonoss\n";
                }
                ...
                if (thisJSONObject.containsValue("customertypenamec") && !thisJSONObject.containsKey("hide")) {
                    Afield += "\t\t\t\t\t,(SELECT d.namec FROM dat_filedata AS d   WHERE d.isleaf = TRUE AND d.isdelete = false AND d.fkcode = 95 AND d.code = (select customertype from sys_corporation where id " +
                            "=t.customerid limit 1)) AS customertypenamec\n";
                }
                ...
                if (thisJSONObject.containsValue("tradewaystr") && !thisJSONObject.containsKey("hide")) {
                    Afield += "\t\t\t\t\t,(CASE WHEN t.ldtype = 'F' THEN 'FOB' WHEN t.ldtype = 'C' THEN 'CIF' WHEN t.ldtype = 'D' THEN 'DDP' WHEN t.ldtype = 'E' THEN 'EXW' WHEN t.ldtype = 'A' THEN 'FCA' WHEN t" +
                            ".ldtype = 'G' THEN 'DDU' WHEN t.ldtype = 'H' THEN 'DAP' ELSE '' END) AS tradewaystr\n";
                }
                ...
                // neo 控制费用显示
                String filterJobs =
                        "and (EXISTS(SELECT 1 FROM f_fina_arap_filter('jobid='||t.id||';userid=" + sysUser.getId() + ";corpidcurrent=" + sysUser.getSysCorporation().getId() +
                                "') x WHERE x.id = aa.id)" +
                                " OR (aa.corpid = 157970752274 AND EXISTS(SELECT 1 FROM fina_jobs WHERE id = t.id AND isdelete = FALSE AND corpid = ANY(SELECT COALESCE(corpid,0) from sys_user_corplink WHERE " +
                                "userid =" +
                                " " + sysUser.getId() + " and ischoose = true))" +
                                "AND (CASE WHEN COALESCE(f_sys_config('fina_arap_feehide'),'N') = 'Y' " +
                                "	THEN	" +
                                "		(CASE WHEN EXISTS(SELECT 1 FROM dat_feeitem WHERE id = aa.feeitemid AND ishidden = TRUE) THEN f_checkright('CheckTheCostOfTheShadow'," + sysUser.getId() +
                                ") > 0 ELSE TRUE END)	" +
                                "	ELSE " +
                                "		TRUE	" +
                                "	END)))";
                filterJobs += "\nAND aa.jobid = ANY(SELECT t.id UNION (SELECT x.id FROM fina_jobs x where x.parentid = t.id AND x.isdelete = false))";
                if (thisJSONObject.containsValue("amt_ar_cny") && !thisJSONObject.containsKey("hide")) {
                    Afield += "\t\t\t\t\t,(SELECT SUM(COALESCE(aa.amount,0)) FROM fina_arap aa WHERE t.id = aa.jobid AND aa.araptype ='AR' AND aa.currency = 'CNY' AND aa.rptype <> 'O' " +
                            "AND aa.isdelete = FALSE " + filterJobs + ") AS amt_ar_cny\n";
                }
                if (thisJSONObject.containsValue("amt_ap_cny") && !thisJSONObject.containsKey("hide")) {
                    Afield += "\t\t\t\t\t,(SELECT SUM(COALESCE(aa.amount,0)) FROM fina_arap aa WHERE t.id = aa.jobid AND aa.araptype ='AP' AND aa.currency = 'CNY'  AND aa.rptype <> 'O' " +
                            "AND aa.isdelete = FALSE " + filterJobs + ") AS amt_ap_cny\n";
                }
                if (thisJSONObject.containsValue("amt_cny_profit") && !thisJSONObject.containsKey("hide")) {
                    Afield += "\t\t\t\t\t,(SELECT SUM(COALESCE(aa.amount,0)) FROM fina_arap aa WHERE t.id = aa.jobid AND aa.araptype ='AR' AND aa.currency = 'CNY' AND aa.rptype <> 'O' " +
                            "AND aa.isdelete = FALSE " + filterJobs + ") " +
                            "-" +
                            "(SELECT SUM(COALESCE(aa.amount,0)) FROM fina_arap aa WHERE t.id = aa.jobid AND aa.araptype ='AP' AND aa.currency = 'CNY'  AND aa.rptype <> 'O' " +
                            "AND aa.isdelete = FALSE " + filterJobs + ") " +
                            "AS amt_cny_profit\n";
                }
                if (thisJSONObject.containsValue("amt_ar_usd") && !thisJSONObject.containsKey("hide")) {
                    Afield += "\t\t\t\t\t,(SELECT SUM(COALESCE(aa.amount,0)) FROM fina_arap aa WHERE t.id = aa.jobid AND aa.araptype ='AR' AND aa.currency = 'USD'  AND aa.rptype <> 'O' " +
                            "AND aa.isdelete = FALSE " + filterJobs + ") " +
                            "AS amt_ar_usd\n";
                }
                if (thisJSONObject.containsValue("amt_ap_usd") && !thisJSONObject.containsKey("hide")) {
                    Afield += "\t\t\t\t\t,(SELECT SUM(COALESCE(aa.amount,0)) FROM fina_arap aa WHERE t.id = aa.jobid AND aa.araptype ='AP' AND aa.currency = 'USD'  AND aa.rptype <> 'O' " +
                            "AND aa.isdelete = FALSE " + filterJobs + ") " +
                            "AS amt_ap_usd\n";
                }
                if (thisJSONObject.containsValue("amt_usd_profit") && !thisJSONObject.containsKey("hide")) {
                    Afield += "\t\t\t\t\t,(SELECT SUM(COALESCE(aa.amount,0)) FROM fina_arap aa WHERE t.id = aa.jobid AND aa.araptype ='AR' AND aa.currency = 'USD'  AND aa.rptype <> 'O' " +
                            "AND aa.isdelete = FALSE " + filterJobs + ") " +
                            "-" +
                            "(SELECT SUM(COALESCE(aa.amount,0)) FROM fina_arap aa WHERE t.id = aa.jobid AND aa.araptype ='AP' AND aa.currency = 'USD'  AND aa.rptype <> 'O' " +
                            "AND aa.isdelete = FALSE " + filterJobs + ") " +
                            "AS amt_usd_profit\n";
                }
                if (thisJSONObject.containsValue("profit_usd") && !thisJSONObject.containsKey("hide")) {
                    Afield += "\t\t\t\t\t,(SELECT COALESCE (SUM(f_amtto((CASE WHEN isamend = TRUE AND (CASE WHEN COALESCE(f_sys_config('fina_arap_porfit_isamend_jobdate'),'N') = 'Y' THEN TRUE ELSE FALSE END) = FALSE THEN a.arapdate ELSE t.jobdate END), a.currency, 'USD', a.amount)), 0) \n" +
                            " FROM fina_arap a WHERE isdelete = FALSE AND araptype = 'AR' AND jobid = t.id " + corpidStr + ") " +
                            "-" +
                            "(SELECT COALESCE (SUM(f_amtto((CASE WHEN isamend = TRUE AND (CASE WHEN COALESCE(f_sys_config('fina_arap_porfit_isamend_jobdate'),'N') = 'Y' THEN TRUE ELSE FALSE END) = FALSE THEN a.arapdate ELSE t.jobdate END), a.currency, 'USD', a.amount)), 0) \n" +
                            " FROM fina_arap a WHERE isdelete = FALSE AND araptype = 'AP' AND jobid = t.id " + corpidStr + ") " +
                            "AS profit_usd\n";
                }
                ...
                if (thisJSONObject.containsValue("clearancecusname") && !thisJSONObject.containsKey("hide")) {
                    Sfield += ",(SELECT string_agg((a.abbr)::text, ','::text) AS string_agg \n" +
                            ...
                            "\tAND (EXISTS ( SELECT 1 FROM bus_customs WHERE ((((bus_customs.clearancecusid = a.id) AND ((bus_customs.clstype)::text = 'I'::text)) AND (bus_customs.jobid = t.id)) AND " +
                            "(bus_customs.isdelete = false)))))\n" +
                            ") AS clearancecusname";
                ...
                if (thisJSONObject.containsValue("busstatusdesc") && !thisJSONObject.containsKey("hide")) {
                    Sfield += "\t\t\t\t,(SELECT namec FROM dat_filedata x , bus_shipping y WHERE x.isdelete = false AND x.fkcode = 320 AND x.isleaf = TRUE AND x.code = y.busstatus AND y.jobid = t.id LIMIT 1) AS" +
                            " busstatusdesc\n";
                ...
                if (thisJSONObject.containsValue("shipjoinos") && !thisJSONObject.containsKey("hide")) {
                    Sfield += "\t\t\t\t,(SELECT string_agg((j.nos)::text, ''::text) AS string_agg \n" +
                            "FROM bus_shipjoin j WHERE ((j.id = ANY (ARRAY( SELECT l.shipjoin FROM bus_shipjoinlink l, bus_shipping s_1 WHERE ((l.shipid = s_1.id) AND (s_1.jobid = t.id))))) AND (j.isdelete = " +
                            "false))\n" +
                            ") AS shipjoinos\n";
                ...
                if (thisJSONObject.containsValue("invoceno") && !thisJSONObject.containsKey("hide")) {
                    Sfield += "\t\t\t\t,(SELECT string_agg(DISTINCT (x.invoiceno)::text, ','::text) AS string_agg \n" +
                            ...
                            "WHERE ((((((y.jobid = t.id) AND (x.isdelete = false)) AND (y.isdelete = false)) AND (y.invoiceid = x.id)) AND (x.invoiceno IS NOT NULL)) AND ((x.invoiceno)::text <> ''::text))) AS " +
                            "invoceno\n";
            ...
            String permissionsWhere = "AND ( t.saleid = " + sysUser.getId()
                    ...
                    "AND t.corpid <> " + sysUser.getSysCorporation().getId() + " AND " + sysUser.getSysCorporation().getId() + " = ANY(SELECT t.corpidop UNION SELECT t.corpidop2 UNION (SELECT corpid FROM " +
                    "fina_corp c WHERE c.jobid = t.id AND c.isdelete =FALSE)))"
                    + "\n	OR EXISTS"
                    + "\n				(SELECT "
                    + "\n					1 "
            ...
            String corpfilterWhere = " AND (EXISTS(SELECT 1 FROM sys_user_corplink x WHERE x.corpid = ANY(SELECT t.corpid UNION SELECT t.corpidop UNION SELECT t.corpidop2 UNION SELECT corpid FROM fina_corp c " +
                    "WHERE c.jobid = t.id AND c.isdelete = FALSE) AND x.ischoose = TRUE AND userid =" + sysUser.getId() + ") OR COALESCE(t.saleid,0) <= 0)";
