...
@ManagedBean(name = "pages.module.report.arap.accountinfoBean", scope = ManagedBeanScope.REQUEST)
public class AccountInfoBean extends GridView {
	...
	public void report() {
		...
		String openUrl = rpturl
				+ "/reportJsp/showReport.jsp?raq=/static/arap/DZ_TZ.raq";
		AppUtils.openWindow("_apAllCustomReport", openUrl + getArgs());
	}

	private String getArgs() {
		//System.out.print("ppccType" + ppccType);
		SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd");
		String arg = "&datefm=" + df.format(dateFrom) + "&dateto="
				+ df.format(dateTo) + "&customerid=" + customerids + "&ppcc=" + ppccType;
		return arg;
	}
