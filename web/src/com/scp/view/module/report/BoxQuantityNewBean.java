...
@WebServlet("/boxquantitynew")

@ManagedBean(name = "pages.module.report.boxQuantityNewBean")
public class BoxQuantityNewBean extends BaseServlet {
    ...
    public BaseView BusCommerceJobId(HttpServletRequest request) {
        ...
        try {
            ...
            for (int i = 0; i < columndefsJSONArray.size(); i++) {
                ...
                if (thisJSONObject.containsValue("sopcompany") && !thisJSONObject.containsKey("hide")) {
                    Afield += "\t\t\t\t\t,(SELECT string_agg((SELECT abbr FROM sys_corporation WHERE id = x.corpid AND isdelete = FALSE limit 1),',') FROM fina_corp x WHERE x.isdelete = FALSE and jobid = t.id " +
                            "and corpoptype ='HY') as sopcompany\n";
                }
