...
@ManagedBean(name = "pages.module.lclload.trainlcldtlBean", scope = ManagedBeanScope.REQUEST)
public class TrainLclDtlBean extends MastDtlView {
	...
	public Map getQryClauseWhere(Map<String, Object> queryMap) {
		...
		//qry += "\nAND EXISTS(SELECT 1 FROM del_loadtl x ,wms_outdtl y,wms_out z WHERE x.outdtlid= y.id AND z.inid = p.id AND x.loadid = "+ mPkVal+")" ;
		m.put("qry", qry);
		
		String filter = "\nAND i.id = ANY(SELECT DISTINCT z.inid FROM  del_loadtl x , wms_outdtlref y , wms_indtl z WHERE x.loadid = "+ mPkVal+" AND x.outdtlid = y.outdtlid AND y.indtlid = z.id AND x.isdelete = false)";
		m.put("filter", filter);
		return m;
	}
...
//		chooseGoods.setDisabled(issubmit);
	...
	public void delBatch() {
		...
		try {
			serviceContext.delLoaddtlMgrService.delBatch(mPkVal,ids);
