...
@ManagedBean(name = "pages.module.price.addfeeBean", scope = ManagedBeanScope.REQUEST)
public class AddFeeBean extends GridView{
    	...
    	if(!StrUtils.isNull(data)){
    		...
    		for (String id : ids) {
				List<PriceFclFeeadd> removeList = serviceContext.pricefclfeeaddMgrService.pricefclfeeaddDao
									.findAllByClauseWhere("fclid="+id + " AND isdelete = false AND feeitemid="+fclFeeadd.getFeeitemid());
				for (PriceFclFeeadd remove : removeList) {
	    			...
	    			priceFclFeeadd.setCntypeid(fclFeeadd.getCntypeid());
    				serviceContext.pricefclfeeaddMgrService.pricefclfeeaddDao.modify(remove);
				}
			}
            this.alert("OK");
            update.markUpdate(true, UpdateLevel.Data, "templateComBo");
        	...
        	for(FclFeeAdd fclFeeadd:list){
	    		for (String id : ids) {
					List<PriceFclFeeadd> removeList = serviceContext.pricefclfeeaddMgrService.pricefclfeeaddDao
										.findAllByClauseWhere("fclid="+id + " AND isdelete = false AND feeitemid="+fclFeeadd.getFeeitemid());
					for (PriceFclFeeadd remove : removeList) {
						serviceContext.pricefclfeeaddMgrService.removes(remove.getId());
					}
				}
        	}
            this.alert("OK");
        		...
        		for (String id : ids) {
        			for (FclFeeAdd fclFeeadd : list) {
        				List<PriceFclFeeadd> addfees = serviceContext.pricefclfeeaddMgrService.pricefclfeeaddDao
							.findAllByClauseWhere("fclid="+id + " AND isdelete = false AND feeitemid="+fclFeeadd.getFeeitemid());//找到此运价包含此费用名称的附加费
        				if(addfees.size()>0){//找到就把此运价的所有这个名称的附加费更新
	        				for (PriceFclFeeadd addfee : addfees) {
	        					...
	        					addfee.setFeeitemcode(fclFeeadd.getFeeitemcode());
	        					addfee.setFeeitemname(fclFeeadd.getFeeitemname());
	        					addfee.setPpcc(fclFeeadd.getPpcc());
	        					addfee.setUnit(fclFeeadd.getUnit());
	        					addfee.setFclid(Long.valueOf(id));
	        					addfee.setInputer(AppUtils.getUserSession().getUsercode());
	        					addfee.setInputtime(Calendar.getInstance().getTime());
    	...
    	if(!StrUtils.isNull(data)){
        	...
        	if(!StrUtils.isNull(batchRefIds)){
        		...
        		for (String id : ids) {
        			...
        			List<PriceFclFeeadd> removeList = serviceContext.pricefclfeeaddMgrService.pricefclfeeaddDao.findAllByClauseWhere("fclid="+id + " AND isdelete = false");
        			for (PriceFclFeeadd remove : removeList) {
        				serviceContext.pricefclfeeaddMgrService.removes(remove.getId());
					}
				}
        		...
        		for (FclFeeAdd fclFeeadd : list) {
                	//AppUtils.debug("priceFclFeeadd:  = " + fclFeeadd.getCurrency());//从list中取得内容即可
                	
                	PriceFclFeeadd priceFclFeeadd = new PriceFclFeeadd();
                	if(fclFeeadd.getId() > 0){
                		priceFclFeeadd = serviceContext.pricefclfeeaddMgrService.pricefclfeeaddDao.findById(fclFeeadd.getId());
                	...
                	priceFclFeeadd.setCurrency(fclFeeadd.getCurrency());
//                	priceFclFeeadd.setFclid(fclFeeadd.getFclid());
                	if(fclFeeadd.getFeeitemid() != null)priceFclFeeadd.setFeeitemid(fclFeeadd.getFeeitemid());
                	priceFclFeeadd.setFeeitemcode(fclFeeadd.getFeeitemcode());
                	priceFclFeeadd.setFeeitemname(fclFeeadd.getFeeitemname());
        			priceFclFeeadd.setPpcc(fclFeeadd.getPpcc());
        			priceFclFeeadd.setUnit(fclFeeadd.getUnit());
        			...
        			if(fclFeeadd.getCntypeid() != null)priceFclFeeadd.setCntypeid(fclFeeadd.getCntypeid());
        			priceFclFeeadd.setInputer(AppUtils.getUserSession().getUsercode());
        			priceFclFeeadd.setInputtime(Calendar.getInstance().getTime());
        			priceFclFeeadd.setCorpid(AppUtils.getUserSession().getCorpid());
        			if(StrUtils.isNull(fclFeeadd.getFeeitemcode()) || StrUtils.isNull(fclFeeadd.getFeeitemname())){
