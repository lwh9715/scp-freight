...
@ManagedBean(name = "pages.module.price.mgrlcltrainBean", scope = ManagedBeanScope.REQUEST)
public class MgrlcltrainBean extends EditGridFormView{
	...
	protected void doServiceSave() {
		try {
			priceTrainLclService.priceTrainLclDao.createOrModify(selectedRowData);
	...
	public void importDataBatch() {
		if (StrUtils.isNull(importDataText)) {
			...
			try {
				...
				String callFunction = "f_imp_price_train_lcl";
				String args = -100 + ",'"
	...
	public void release() {
		...
		priceTrainLclService.release(ids);
		MessageUtils.alert("OK!");
		this.refresh();
	}
	...
	public void del() {
		...
		priceTrainLclService.removes(ids);
		this.refresh();
	}
	...
	public void addForm(){
		this.pkVal = -1L;
		this.selectedRowData = new PriceTrainLcl();
		update.markUpdate(true, UpdateLevel.Data, "editPanel");
	...
	public void addcopy() {
		...
		PriceTrainLcl old = priceTrainLclService.priceTrainLclDao.findById(id);
		this.addForm();
		...
		this.selectedRowData.setCostpricel(old.getCostpricel());
		this.selectedRowData.setBasepricel(old.getBasepricel());
		this.selectedRowData.setCostpriceh(old.getCostpriceh());
		this.selectedRowData.setBasepriceh(old.getBasepriceh());
