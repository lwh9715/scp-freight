...
@ManagedBean(name = "pages.module.airtransport.airporthandleBean", scope = ManagedBeanScope.REQUEST)
public class AirporthandleBean extends GridView{
...
//				+ "\n)";
		...
		if("Q".equals(type)){
			...
			sql +="\nAND EXISTS(select 1 from _bpm_task where refid = t.id::text and state = '9' AND taskname IN ('跟单','机场处理'))";
		}else if("N".equals(type)){//待处理
			sql +="\nAND EXISTS(select 1 from _bpm_task where refid = t.id::text and state <> '3' and state <> '9' AND taskname IN ('跟单','机场处理'))";
		}
		
		if(!StrUtils.isNull(qryGoodsname)){
			sql += "\nAND EXISTS(SELECT 1 FROM bus_air b WHERE goodsname ILIKE '%"+qryGoodsname+"%' AND b.jobid = t.id)";
		}
	...
	@Action
	public void warehousdraft(){
		String rpturl = AppUtils.getRptUrl();
		try{
			String jobid = AppUtils.getReqParam("jobid");
			String openUrl = rpturl + "/reportJsp/showReport.jsp?raq=/airtransport/szairport.raq&id="+Long.valueOf(jobid)+ "&userid="
			+ AppUtils.getUserSession().getUserid();
			AppUtils.openWindow("_warehousdraft", openUrl);
		} catch (Exception e) {
	...
	@Action
	public void warehousdraftCZ(){
		String rpturl = AppUtils.getRptUrl();
		try{
			String jobid = AppUtils.getReqParam("jobid");
			String openUrl = rpturl + "/reportJsp/showReport.jsp?raq=/airtransport/szairportCZ.raq&id="+Long.valueOf(jobid)+ "&userid="
			+ AppUtils.getUserSession().getUserid();
			AppUtils.openWindow("_warehousdraftCZ", openUrl);
		} catch (Exception e) {
			MessageUtils.showException(e);
		}
	...
	@Action
	public void arrivalnoticeGZ(){
		String rpturl = AppUtils.getRptUrl();
		try{
			String jobid = AppUtils.getReqParam("jobid");
			String openUrl = rpturl + "/reportJsp/showReport.jsp?raq=/airtransport/arrivalnoticeGZ.raq&id="+Long.valueOf(jobid)+ "&userid="
			+ AppUtils.getUserSession().getUserid();
			AppUtils.openWindow("_arrivalnoticeGZ", openUrl);
		} catch (Exception e) {
			MessageUtils.showException(e);
		}
	}
	...
	@Action
	public void slinoticeGZ(){
		String rpturl = AppUtils.getRptUrl();
		try{
			String jobid = AppUtils.getReqParam("jobid");
			String openUrl = rpturl + "/reportJsp/showReport.jsp?raq=/airtransport/sliGZ.raq&id="+Long.valueOf(jobid)+ "&userid="
			+ AppUtils.getUserSession().getUserid();
			AppUtils.openWindow("_slinoticeGZ", openUrl);
		} catch (Exception e) {
			MessageUtils.showException(e);
		}
	}
