...
@ManagedBean(name = "pages.module.airtransport.salecheckBean", scope = ManagedBeanScope.REQUEST)
public class SalecheckBean extends GridView{
	...
	public Map getQryClauseWhere(Map<String, Object> queryMap) {
		...
		if("Q".equals(type)){
			...
			sql +="\nAND EXISTS(select 1 from _bpm_task where refid = t.id::text and state = '9' AND taskname IN ('托运单审核'))";
		}else if("N".equals(type)){//待处理
			sql +="\nAND EXISTS(select 1 from _bpm_task where refid = t.id::text and state <> '3' and state <> '9' AND taskname IN ('托运单审核'))";
		}
