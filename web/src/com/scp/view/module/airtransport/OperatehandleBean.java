...
@ManagedBean(name = "pages.module.airtransport.operatehandleBean", scope = ManagedBeanScope.REQUEST)
public class OperatehandleBean extends GridView{
	...
	public Map getQryClauseWhere(Map<String, Object> queryMap) {
		...
		if("Q".equals(type)){
			...
			sql +="\nAND EXISTS(select 1 from _bpm_task where refid = t.id::text and state = '9' AND taskname IN ('客服审核','客服跟单','机场处理'))";
		}else if("N".equals(type)){//待处理
			sql +="\nAND EXISTS(select 1 from _bpm_task where refid = t.id::text and state <> '3' and state <> '9' AND taskname IN ('客服审核','客服跟单','机场处理'))";
		}
