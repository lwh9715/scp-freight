...
@ManagedBean(name = "pages.module.airtransport.report.aircomplexqryBean", scope = ManagedBeanScope.REQUEST)
public class AircomplexqryBean extends GridView{
	...
	public void clearQryKey() {
		...
		agentid = 0L;
		customscompid = 0L;
		Browser.execClientScript("clear()");
		if (qryMap != null) {
			qryMap.clear();
			gridLazyLoad = false;
	...
	public Map getQryClauseWhere(Map<String, Object> queryMap) {
		...
		if("Q".equals(type)){
			...
			sql +="\nAND EXISTS(select 1 from _bpm_task where refid = t.id::text and state = '9' AND taskname IN ('提交','Start'))";
		}else if("N".equals(type)){//待处理
			sql +="\nAND EXISTS(select 1 from _bpm_task where refid = t.id::text and state <> '3' and state <> '9' AND taskname IN ('提交','Start'))";
		}
		
		if(agentid != 0){
			sql += "\nAND EXISTS(select 1 FROM bus_air b WHERE agentid = "+agentid+" AND b.jobid = t.id)";
		}
		
		if(customscompid != 0){
			sql += "\nAND EXISTS(select 1 FROM bus_air b WHERE customscompid = "+customscompid+" AND b.jobid = t.id)";
		}
		...
		String ordersql = "ORDER BY jobdate DESC";
