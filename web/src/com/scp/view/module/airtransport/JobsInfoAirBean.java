...
@ManagedBean(name = "pages.module.airtransport.jobsinfoairBean", scope = ManagedBeanScope.REQUEST)
public class JobsInfoAirBean extends GridView{
		...
		if (!isPostBack) {
			try {
				...
				if(!StrUtils.isNull(id1)) {
					...
					String sql = "SELECT id FROM bpm_processinstance WHERE refid = '"+jobid+"' ORDER BY id LIMIT 1";
					Map m = null;
					try {
						m = serviceContext.daoIbatisTemplate.queryWithUserDefineSql4OnwRow(sql);
						processinstanceid = StrUtils.getMapVal(m, "id");
					} catch (Exception e) {
					}
				}
				...
				update.markUpdate(true, UpdateLevel.Data, "processinstanceid");
	...
	public void getshowbillnos(){
		String sql = 
				"\nselect string_agg('分单号:'||hawbno||'  '||'件数:'||piece||'  '||'付款:'||ppccpaytype, f_newline() ORDER BY hawbno) AS billnosinfo " +
				"\nfrom bus_air_bill " +
				"\nwhere isdelete = false and jobid = "+this.jobid+";";
		try{
			Map m = serviceContext.daoIbatisTemplate.queryWithUserDefineSql4OnwRow(sql);
			if(m!=null&&m.get("billnosinfo")!=null){
				showbillnos = m.get("billnosinfo").toString();
	...
	public void initjobs() {
		...
		if (!StrUtils.isNull(id)) {
			...
			if("S".equals(selectedRowData.getJobtype())){
				...
				/*if(this.airRowData.getCarrierid() != null && this.airRowData.getCarrierid()>0){
					String sql = "select abbr from sys_corporation where isdelete = false and isairline = true and id = "+airRowData.getCarrierid();
					Map map = daoIbatisTemplate.queryWithUserDefineSql4OnwRow(sql);
					carrier = map.get("abbr").toString();
				}*/
				if(this.airRowData.getAgentid() != null && this.airRowData.getAgentid()>0){
					String sql = "SELECT abbr,* FROM sys_corporation WHERE isagent = TRUE AND isdelete = FALSE and id = "+airRowData.getAgentid();
					Map map = daoIbatisTemplate.queryWithUserDefineSql4OnwRow(sql);
					agent = map.get("abbr").toString();
				}
	...
	public void initBillInfo(){
		...
		"\n( "+
		"\nSELECT  "+
		"\n	 COALESCE(mawbno,'') AS mawbno "+
		"\n	, COALESCE(cnortitle,'') AS cnortitle "+
		"\n	, COALESCE(cneetitle,'') AS cneetitle "+
		"\n	,flightno1 || '/' || flightdate1 AS flightinfo "+
		"\n	, COALESCE(goodsdesc,'') AS goodsname  "+
		"\n	, COALESCE(markno,'') AS marks  "+
		"\n	, COALESCE(other2,'') AS notify  "+
		"\n	, (SELECT COALESCE(s.doccode,'') FROM air_bookingcode s WHERE b.agentid = s.airlineid AND s.isdelete = false limit 1) AS handlinginformation  "+
		"\nFROM bus_air b "+
		"\nWHERE b.isdelete = FALSE AND jobid = "+jobid+" "+
		"\nUNION "+
		"\nSELECT  "+
		"\n	  COALESCE(hawbno,'') AS mawbno "+
		"\n	, COALESCE(cnortitle,'') AS cnortitle "+
		"\n	, COALESCE(cneetitle,'') AS cneetitle "+
		"\n	,flightno1 || '/' || flightdate1 AS flightinfo "+
		"\n	, COALESCE(goodsdesc,'') AS goodsname  "+
		...
		try {
			Map m = serviceContext.daoIbatisTemplate.queryWithUserDefineSql4OnwRow(sql);
			billInfo = StrUtils.getMapVal(m, "json");
			update.markUpdate(true, UpdateLevel.Data, "billInfo");
		} catch (Exception e) {
		}
	}
}
