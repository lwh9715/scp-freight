...
@ManagedBean(name = "pages.module.finance.payapplycheckBean", scope = ManagedBeanScope.REQUEST)
public class PayApplyCheckBean extends GridView {
...
//				ispublic = (m.get("ispublic")!=null&&((String) m.get("ispublic")).equals("0")?false:true);
	...
	public void grid_ondblclick() {
		try {
			...
			String winId = "_pay_apply_dtlcheck";
			String url = "./payapplydtlcheck.xhtml?araptype=AP&id="+Long.valueOf(ids[0]);
			AppUtils.openNewPage(url);
		} catch (Exception e) {
	...
	public void cancelCheckBatch() {
		...
		try {
			...
			for(int i =0;i<ids.length;i++){
				String id = ids[i];
				newIds[i] = id.substring(0, id.indexOf("."));
			}
			
			serviceContext.reqMgrService.updateCancelCheck(newIds);
	...
	public Map getQryClauseWhere(Map<String, Object> queryMap) {
		...
		if(!inputername.equals("")){
			qry+="\n AND (EXISTS(SELECT 1 FROM sys_user x,fina_rpreq y where y.id = t.id AND x.code = y.inputer and x.isdelete = false AND x.namec LIKE '%"+
					inputername+"%') OR t.inputer LIKE '%"+inputername+"%')";
		}
	...
	@Bind
	private UIIFrame uploadFileIframe;
	
	@Action
	public void uploadBankBill(){
		try {
			String[] ids = this.grid.getSelectedIds();
			if (ids == null || ids.length == 0) {
			...
			String[] linkidsArray = new String[set.size()];
			int index = 0;
			for (String key : set) {
				linkidsArray[index++] = key;
			}
			String linkids = StrUtils.array2List(linkidsArray);
			uploadFileIframe.load(AppUtils.getContextPath()+ "/pages/module/common/attachmentbatch.xhtml?linkid="+ linkids);
			Browser.execClientScript("uploadFileWindowJsVar.show();");
		} catch (Exception e) {
			MessageUtils.showException(e);
			e.printStackTrace();
		}
