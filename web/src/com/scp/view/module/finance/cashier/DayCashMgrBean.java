...
import com.scp.model.finance.fs.FsCash;
import com.scp.util.AppUtils;
import com.scp.util.MessageUtils;
import com.scp.util.SaasUtil;
import com.scp.view.comp.GridFormView;

@ManagedBean(name = "pages.module.finance.cashier.daycashmgrBean", scope = ManagedBeanScope.REQUEST)
public class DayCashMgrBean extends GridFormView {
	...
	@Override
	public void add() {
		selectedRowData = new FsCash();
		selectedRowData.setCorpid(AppUtils.getUserSession().getCorpid());
		selectedRowData.setCashdate(Calendar.getInstance().getTime());
		selectedRowData.setActsetid(this.actsetid);
	...
	protected void doServiceFindData() {
		selectedRowData = this.serviceContext.fsCashMgrService.fsCashDao.findById(this.pkVal);
	}


	@Override
	protected void doServiceSave() {
		this.serviceContext.fsCashMgrService.saveData(selectedRowData);
		this.alert("OK");
	}
	...
	public void deleter(){
		...
		if (ids == null || ids.length == 0 || ids.length > 1) {
			...
			this.serviceContext.fsCashMgrService.removeDate(getGridSelectId());
			refresh();
		}
	}
	...
	public void del() {
		if(selectedRowData.getId()==0){
			...
			this.serviceContext.fsCashMgrService.removeDate(selectedRowData.getId());
		refresh();
		this.add();
		this.alert("OK");
		}
