...
@ManagedBean(name = "pages.module.finance.doc.indtl_sapBean", scope = ManagedBeanScope.REQUEST)
public class IndtlSapBean extends GridFormView {
	...
	@Bind
	@SaveState
	public String selectArrayData = "[]";//往来单位等下拉框JSON数据
	@Bind
	@SaveState
	...
	public void init() {
		...
		if (!StrUtils.isNull(id)) {
			...
			this.jsonData = getVchdtlByVchId(mPkVal);
			//this.jsonData = serviceContext.vchdtlMgrService.getVchdtlByVchId(mPkVal);
			
//			this.selectArrayData = this.serviceContext.fsAstMgrService.fsAstDao.executeQuery("");
			this.initRate();
	 ...
	 * @return
	 */
	public String getVchdtlByVchId(Long vchid){
		String sql = 
					"\nSELECT"+
					...
					"\n				v.astid3,"+
					"\n				v.astid4,"+
					"\n				(SELECT x.code FROM fs_ast x WHERE x.id = v.astid AND x.isdelete = false LIMIT 1) AS astcode,"+
					"\n				(SELECT x.code FROM fs_ast x WHERE x.id = v.astid2 AND x.isdelete = false LIMIT 1) AS astcode2,"+
					"\n				(SELECT x.code FROM fs_ast x WHERE x.id = v.astid3 AND x.isdelete = false LIMIT 1) AS astcode3,"+
					"\n				(SELECT x.code FROM fs_ast x WHERE x.id = v.astid4 AND x.isdelete = false LIMIT 1) AS astcode4,"+
					"\n				(SELECT x.name FROM fs_ast x WHERE x.id = v.astid AND x.isdelete = FALSE LIMIT 1) AS astcodename," +
					"\n				(SELECT x.name FROM fs_ast x WHERE x.id = v.astid2 AND x.isdelete = FALSE LIMIT 1) AS astcode2name," +
					"\n				(SELECT x.name FROM fs_ast x WHERE x.id = v.astid3 AND x.isdelete = FALSE LIMIT 1) AS astcode3name," +
					"\n				(SELECT x.name FROM fs_ast x WHERE x.id = v.astid4 AND x.isdelete = FALSE LIMIT 1) AS astcode4name," +
					"\n				f_fs_vchdtl_getactdesc('id='::text || v.id) AS actdesc,"+
					"\n			    (SELECT x.cyid FROM fs_act x WHERE x.id = v.actid AND x.isdelete = FALSE) AS actcy"+
					"\n			FROM"+
					"\n				fs_vchdtl v"+
					"\n			WHERE"+
					"\n				v.isdelete = false" +
		...
		//DaoIbatisTemplate daoIbatisTemplate = (DaoIbatisTemplate) AppUtilBase.getBeanFromSpringIoc("daoIbatisTemplate");
...
//	public String jsonDataStr = "''";
	...
	private void saveAjaxSubmit() {
		...
		try{
			...
			jsonData = getVchdtlByVchId(mPkVal);
...
//	}
	...
	protected GridDataProvider getDataProvider4(){
		return new GridDataProvider() {
			...
			public Object[] getElements() {
				String sqlId = "pages.module.finance.doc.indtl_sapBean.gridVchdesc.page";
			...
			public int getTotalCount() {
				String sqlId = "pages.module.finance.doc.indtl_sapBean.gridVchdesc.count";
	...
	public void pageUp(){
		...
		String whereSql = "SELECT id FROM _fs_vch_main WHERE "+seach+" AND isdelete = FALSE AND srctype != 'I' AND actsetid = "
