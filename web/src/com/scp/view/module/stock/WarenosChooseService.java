 ...
 * 
 */
@ManagedBean(name = "warenoschooseserviceBean", scope = ManagedBeanScope.SESSION)
public class WarenosChooseService {
	...
	public void qryNos(String popQryKey) {
		...
		qrySql = 
			"\nAND " +
			"\n	(	" +
			"\n			UPPER(nos) LIKE '%" + this.popQryKey + "%' " +			
			"\n		 OR UPPER(customerabbr) LIKE '%" + this.popQryKey + "%'" +
			"\n		 OR UPPER(refno) LIKE '%" + this.popQryKey + "%'" +
