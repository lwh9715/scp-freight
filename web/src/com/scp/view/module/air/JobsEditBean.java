...
@ManagedBean(name = "pages.module.air.jobseditBean", scope = ManagedBeanScope.REQUEST)
...
//		if(this.selectedRowData!=null && this.selectedRowData.getDeptid()!=null){
...
//		}
	...
	public void beforeRender(boolean isPostBack) {
		if (!isPostBack) {
			...
			initCtrl();
			this.commonCheck();
			if (this.selectedRowData.getCustomerid() != null && !"".equals(this.selectedRowData.getCustomerid())) {
				this.checkCustomerQuota(this.selectedRowData.getCustomerid().toString());
			}
...
//	}
	...
	public void showShipping() {
		if (this.mPkVal == -1l) {
			...
			if(StrUtils.isNull(url) || url.indexOf("blank.xhtml") > 1){
				//airIframe.load("../air/air.xhtml?id=" + this.mPkVal);
				airIframe.setSrc("../air/air.xhtml?id=" + this.mPkVal);
...
//				
	...
	public void showTruck() {
		if (this.mPkVal == -1l) {
			...
			truckIframe.load("./bustruck.xhtml?id=" + this.mPkVal);
		}
	}
...
//		}
	...
	public void showConstruct() {
		if (this.mPkVal == -1l) {
			...
			constructIframe.load("../air/jobschild.xhtml?id=" + this.mPkVal);
		}
	}
...
//			this.deptids = getqueryDepartid();
	...
	public void arrears(String customerid){
		...
		try {
			//卓航
			Map map = this.serviceContext.daoIbatisTemplate.queryWithUserDefineSql4OnwRow("Select corpid FROM SYS_USER WHERE isdelete = FALSE AND id = "+loguserid);
			//前海国际事业部
			...
			if(!"13632274".equals(String.valueOf(map.get("corpid"))) && !"1464156732274".equals(String.valueOf(deptmap.get("deptid")))
			...
			String sqlQry = "SELECT * FROM f_sys_corporation_checkamtowe2('"+urlArgs2+"')";
			Map m = this.serviceContext.daoIbatisTemplate.queryWithUserDefineSql4OnwRow(sqlQry);
			String str = m.get("f_sys_corporation_checkamtowe2").toString();
			if(!"1".equals(str)){
...
//			this.deptids = getqueryDepartid();
	...
	public Map getPageNumber(Long jobid,int pageNumber){
		...
		String seach = ConfigUtils.findUserSysCfgVal("pages.module.air.jobsBean.search", AppUtils.getUserSession().getUserid());
		if(!StrUtils.isNull(seach)){
			sbsql.append(seach);
		}
		
		String ordersql = AppUtils.getUserColorder("pages.module.air.jobsBean.grid");
	...
	public void cusalesSubmit() {
		...
		try {
			if (cusalesconfirm == true) {
				serviceContext.daoIbatisTemplate.updateWithUserDefineSql("Update fina_jobs SET cusalesconfirm = "
						+ cusalesconfirm + ",cusalesid = " + AppUtils.getUserSession().getUserid() + ",updater = '"
						+ AppUtils.getUserSession().getUsercode() + "' WHERE isdelete = false and id = " + selectedRowData.getId());
				...
				serviceContext.daoIbatisTemplate.updateWithUserDefineSql("Update fina_jobs SET cusalesconfirm = "
						+ cusalesconfirm + ",cusalesid = NULL, updater = '" + AppUtils.getUserSession().getUsercode()
						+ "' WHERE isdelete = false and id = " + selectedRowData.getId());
				cusales = "";
	...
	@Bind
	@SaveState
	public String owequota = "";

	@Bind
	@SaveState
	public String owequotacss = "padding-left:5px;color:red;text-decoration:underline;font-size:small;";
	 ...
	 * @param customerid
	 */
	public void checkCustomerQuota(String customerid) {
		Map<String, String> argsMap = new HashMap<String, String>();
		argsMap.put("customerid", customerid);

		Map map = this.serviceContext.daoIbatisTemplate.queryWithUserDefineSql4OnwRow("select sc.amtowe,sc.daysar," +
				" ((SELECT SUM(f_amtto(a.arapdate::DATE, a.currency, 'CNY', COALESCE(amount, 0))) FROM fina_arap a " +
				"WHERE isdelete = FALSE AND a.araptype = 'AR' AND a.rptype = 'G' AND a.customerid = sc.id) -\n" +
				" (SELECT SUM(f_amtto(a.arapdate::DATE, a.currency, 'CNY', COALESCE(amtstl2, 0))) FROM fina_arap a " +
				"WHERE isdelete = FALSE AND a.araptype = 'AR' AND a.rptype = 'G' AND a.customerid = sc.id))::numeric(18,2) as owequota\n" +
				" from sys_corporation sc where sc.isdelete = false and sc.id = " + customerid);

		this.quota = map.get("amtowe") == null ? "" : map.get("amtowe").toString();
		this.daysar = map.get("daysar") == null ? "" : map.get("daysar").toString();
		this.owequota = map.get("owequota") == null ? "" : map.get("owequota").toString();

		//当欠款超过额度时，标记为红色
		if (!StrUtils.isNull(quota) && !StrUtils.isNull(owequota) && Double.parseDouble(quota)>Double.parseDouble(owequota)) {
			owequotacss = "padding-left:5px;text-decoration:underline;font-size:small;";
		}

		update.markUpdate(true, UpdateLevel.Data, "quota");
		update.markUpdate(true, UpdateLevel.Data, "daysar");
		update.markUpdate(true, UpdateLevel.Data, "owequota");
	}

	@Bind
	public UIWindow showQuotaWindow;

	@Bind
	public UIIFrame showQuotaIFrame;
	...
	public void checkOwe(){
		showQuotaIFrame.setSrc("/scp/pages/module/commerce/customerarapowe.html?id=" + this.selectedRowData.getCustomerid());
		update.markAttributeUpdate(showQuotaIFrame, "src");
		if (showQuotaWindow != null) {
			showQuotaWindow.setTitle(this.selectedRowData.getCustomerabbr() + " - 欠款账单");
			showQuotaWindow.show();
		}
	}
