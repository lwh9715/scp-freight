...
@ManagedBean(name = "pages.module.air.busshippinglclBean", scope = ManagedBeanScope.REQUEST)
public class BusShippinglclBean extends MastDtlView {
...
//	}
    ...
    @SelectItems
    @SaveState
    private List<SelectItem> custcontacts;
	public void ChangCustcontacts(){
		String sql ="select customerabbr from bus_shipping where jobid ="+this.jobid+"";
		this.custcontacts  = serviceContext.busShippingMgrService.busShippingDao.executeQuery(sql);
	}
		 ...
		 if(custcontact !=null) {
        	...
        	if(objs !=null) {
        		for(Object[]obj:objs){
                	...
                	this.selectedRowData.setCustfax(this.Chang(obj));
                	update.markUpdate(true, UpdateLevel.Data, "masterEditPanel");
        	}
        }
    }
    ...
    public List<SelectItem> getAllwarehouse() {
    	try {
    		String whereSql = " WHERE isdelete = FALSE AND code IN ('GZCK','SZCK','YWCK','ZSCK')";
			return CommonComBoxBean.getComboxItems("d.id","d.code ||'/'||  COALESCE(d.namec,'') ||'/'||  COALESCE(d.namee,'')","dat_warehouse AS d",whereSql,"");
		} catch (Exception e) {
			MessageUtils.showException(e);
			return null;
		}
	...
	public void changewarehouse(){
		String warehouse = AppUtils.getReqParam("warehouseid").trim();
		DatWarehouse datWarehouse = serviceContext.warehouseMgrService.datWarehouseDao.findById(Long.valueOf(warehouse));
		selectedRowData.setCusttel(datWarehouse.getTel());
		addreswarehouse = datWarehouse.getAddressc();
		selectedRowData.setCustfax(datWarehouse.getFax());
		this.update.markUpdate(UpdateLevel.Data, "addreswarehouse");
		this.update.markUpdate(UpdateLevel.Data, "custtel");
		this.update.markUpdate(UpdateLevel.Data, "custfax");
	}
}
