...
@ManagedBean(name = "pages.module.air.busairbilleditBean", scope = ManagedBeanScope.REQUEST)
public class BusairbilleditBean extends MastDtlView{
	...
	public void chooseship() {
		Long bus_air_bill_id = -1L;
		if (this.selectedRowData.getId() > 0) {
			bus_air_bill_id = this.selectedRowData.getId();
		}
		try {
			updateGoodsSelects(this.grid.getSelectedIds(), bus_air_bill_id, getairid(this.jobid));
			this.serviceContext.userMgrService.daoIbatisTemplate.queryWithUserDefineSql4OnwRow("SELECT f_bus_goods_createbillinfo('shipid=" + bus_air_bill_id + ";type=bill;');");

			String sql = " isdelete = false AND id =" + this.selectedRowData.getId();
			this.selectedRowData = serviceContext.busAirBillMgrService.busAirBillDao
	...
	public void updateGoodsSelects(String[] ids, Long bus_air_bill_id, Long bus_air_id) {
		if (bus_air_bill_id == null) {
			return;
		}
		String sql = "UPDATE bus_goods SET isselect = FALSE WHERE isdelete = FALSE AND linkid = " + bus_air_id;
		this.serviceContext.busAirMgrService.busAirDao.executeSQL(sql);
		if (ids != null && ids.length > 0) {
			StringBuffer sbsql = new StringBuffer();
			sbsql.append("\n UPDATE bus_goods SET isselect = TRUE WHERE isdelete = FALSE ");
			sbsql.append(" AND id = ANY(array[" + StrUtils.array2List(ids) + "])");
			this.serviceContext.busAirMgrService.busAirDao.executeSQL(sbsql.toString());
		}
	}
