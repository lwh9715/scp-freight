...
@ManagedBean(name = "pages.module.kahang.jobseditBean", scope = ManagedBeanScope.REQUEST)
...
//			this.deptids = getqueryDepartid();
	...
	private void initCtrl() {
		...
		for (String s : AppUtils.getUserRoleModuleCtrl(Module.fina_jobs_kahang.getValue())) {
		...
		for (String s : AppUtils.getUserRoleModuleCtrl(250000300130L)) {
			if(s.endsWith("_add")) {
...
//			update.markUpdate(true, UpdateLevel.Data, "deptcomb");
	...
	public void showShipping() {
		if (this.mPkVal == -1l) {
			...
			shippingIframe.load("../kahang/busland.xhtml?id="+this.mPkVal);
		}
	}
...
//		this.deptids = getqueryDepartid();
	...
	public GridDataProvider getJobnosGridDataProvider() {
		return this.jobsnosChooseService.getJobsnosDataProvider("AND jobtype = 'K'");
	}
