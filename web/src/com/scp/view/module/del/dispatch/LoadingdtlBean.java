...
@ManagedBean(name = "pages.module.del.dispatch.loadingdtlBean", scope = ManagedBeanScope.REQUEST)
public class LoadingdtlBean extends MastDtlView {
	...
	public void isubmitAjaxSubmit() {
		...
		try {
			...
			if (isSubmit) {
				sql = "UPDATE del_load SET isubmit = TRUE ,submitime = '" + date
	...
	public List<SelectItem> getCardesc() {
		try {
			return CommonComBoxBean.getComboxItems("c.id", "c.code||'/'||c.carcolor1||'/'||t.cartype||'/'||COALESCE(d.code,'')||'/'||COALESCE(c.remarks,'')",
					"dat_car c ,dat_cartype t,dat_driver d", "WHERE 1=1 AND c.cartypeid = t.id AND c.driverid = d.id", "ORDER BY c.code");
		} catch (Exception e) {
			MessageUtils.showException(e);
			return null;
		}
	...
	public void load() {
		...
		String url = AppUtils.getContextPath() + "/pages/module/del/dispatch/wmsoutdtlchooser.xhtml?loadid="+this.mPkVal;
