...
@ManagedBean(name = "pages.module.customer.memorialdayeditBean", scope = ManagedBeanScope.REQUEST)
public class MemorialDayEditBean extends GridFormView {
	...
	public void beforeRender(boolean isPostBack) {
		if (!isPostBack) {
			...
			selectedRowData = serviceContext.customerMemorialdayMgrService.sysCorpmemoryDao
					.findById(pkid);
	...
	@Override
	public void save() {
		serviceContext.customerMemorialdayMgrService.saveData(selectedRowData);
		this.alert("OK");
	}
	...
	public void del() {
		if(this.pkid==-1l){
			...
			serviceContext.customerMemorialdayMgrService.removeDatedel(this.pkid);
	...
	@Override
	public void add() {
		selectedRowData = new SysCorpmemory();
		pkid = -1L;
		update.markUpdate(true, UpdateLevel.Data, "editPanel");
	}
