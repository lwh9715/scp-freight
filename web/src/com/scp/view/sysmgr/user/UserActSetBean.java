...
@ManagedBean(name = "pages.sysmgr.user.useractsetBean", scope = ManagedBeanScope.REQUEST)
public class UserActSetBean extends GridSelectView {
	...
	public int[] getGridSelIds() {
		...
		if ("-1".equals(userid)) {
			...
			sql = "\nselect "
					+ "\n(CASE WHEN EXISTS(SELECT 1 FROM fs_actsetctrl WHERE userid = " + userid + " AND actsetid = a.id)"
