...
@ManagedBean(name = "pages.sysmgr.role.rolelinkreportBean", scope = ManagedBeanScope.REQUEST)
public class RoleLinkReportBean extends GridSelectView {

	@Bind
	@SaveState
	private String roleid ="-100";
	...
	public void beforeRender(boolean isPostBack) {
		...
		if (!isPostBack) {
			roleid = (String) AppUtils.getReqParam("id");
	...
	public int[] getGridSelIds() {
		...
		if ("-1".equals(roleid)) {
			return null;
		} else if (!StrUtils.isNull(roleid)) {
			sql = 
				"SELECT " +
	 ...
	 */
	public void delAll(String roleid){
		String sql = "DELETE FROM sys_report_ctrl a WHERE linkid = "+roleid+" AND linktype = 'M' AND EXISTS (SELECT * FROM sys_report x WHERE x.isdelete = FALSE AND x.id = a.reportid AND x.modcode = '"+reportType+"')";
		try {
			serviceContext.accountMgrService.datAccountDao.executeSQL(sql);
		} catch (Exception e) {
