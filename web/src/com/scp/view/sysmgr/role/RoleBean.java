...
@ManagedBean(name = "pages.sysmgr.role.roleBean", scope = ManagedBeanScope.REQUEST)
public class RoleBean extends GridFormView {
...
//		refreshDtlFrame(true);
	...
	public void linkSales() {
		...
		String url = "../user/rolelinksales.xhtml?id="+ids[0];
		linkSalesIFrame.setSrc(url);
		update.markAttributeUpdate(linkSalesIFrame, "src");
		update.markUpdate(true, UpdateLevel.Data, linkSalesWindow);
		linkSalesWindow.setTitle("关联业务员");
		linkSalesWindow.show();
	}
	...
	public void linkReport(){
		...
		String url = "./rolelinkreport.xhtml?id=" + ids[0];
		...
		linkReportSetWindow.setTitle("授权报表权限(组)");
