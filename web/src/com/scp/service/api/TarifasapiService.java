...
import com.scp.dao.api.TarifasapiDao;
...
@Component
public class TarifasapiService {

    @Resource
    public TarifasapiDao tarifasapiDao;

    @Resource
    public DaoIbatisTemplate daoIbatisTemplate;


    public void saveData(TarifasApi data) {
        if (0 > data.getId()) {
            tarifasapiDao.create(data);
        } else {
            tarifasapiDao.modify(data);
        }
    }
    ...
    // 	for (String id : ids) {
    // 		String sql = "\nUPDATE tarifas_api SET isrelease = TRUE , daterelease = NOW() WHERE id = " + id + ";";
    // 		stringBuffer.append(sql);
    // 	}
    // 	tarifasapiDao.executeSQL(stringBuffer.toString());
    // }

    public void removes(String[] ids, String username) {
        StringBuffer stringBuffer = new StringBuffer();
        for (String id : ids) {
            String sql = "\nUPDATE tarifas_api SET isdelete = TRUE, updatetime = NOW(),updater = '" + username + "' WHERE id = " + id + ";";
            stringBuffer.append(sql);
        }
        tarifasapiDao.executeSQL(stringBuffer.toString());
    ...
    // 	for (String id : ids) {
    // 		if("+".equals(amttype)){
    // 			String sql = "\nUPDATE tarifas_api SET " + "cost" + type + " = cost" + type + " + " + amtadd  + " , updater = '"+usercode+"' , updatetime = NOW() WHERE id = " + id + ";";
    // 			stringBuffer.append(sql);
    // 		}else{
    // 			String sql = "\nUPDATE tarifas_api SET " + "cost" + type + " = cost" + type + " - " + amtadd  + " , updater = '"+usercode+"' , updatetime = NOW() WHERE id = " + id + ";";
    // 			stringBuffer.append(sql);
    ...
    // 	for (String id : ids) {
    // 		if("+".equals(amttype)){
    // 			String sql = "\nUPDATE tarifas_api SET " + "base" + type + " = base" + type + " + " + amtadd  + " , updater = '"+usercode+"' , updatetime = NOW() WHERE id = " + id + ";";
    // 			stringBuffer.append(sql);
    // 		}else{
    // 			String sql = "\nUPDATE tarifas_api SET " + "base" + type + " = base" + type + " - " + amtadd  + " , updater = '"+usercode+"' , updatetime = NOW() WHERE id = " + id + ";";
    // 			stringBuffer.append(sql);
    ...
    // 	StringBuffer stringBuffer = new StringBuffer();
    // 	for (String id : ids) {
    // 		String sql = "\nUPDATE tarifas_api SET " + "cost" + type + " = " + amtadd  + " , updater = '"+usercode+"' , updatetime = NOW() WHERE id = " + id + ";";
    // 		stringBuffer.append(sql);
    // 	}
    // 	tarifasapiDao.executeSQL(stringBuffer.toString());
    //
    ...
    // 	StringBuffer stringBuffer = new StringBuffer();
    // 	for (String id : ids) {
    // 		String sql = "\nUPDATE tarifas_api SET " + "base" + type + " = " + amtadd  + " , updater = '"+usercode+"' , updatetime = NOW() WHERE id = " + id + ";";
    // 		stringBuffer.append(sql);
    // 	}
    // 	tarifasapiDao.executeSQL(stringBuffer.toString());
    //
    ...
    // 	for (String id : ids) {
    // 		if("+".equals(amttype)){
    // 			String sql = "\nUPDATE tarifas_api SET pieceother=pieceother+" + amtadd  + " , updater = '"+usercode+"' , updatetime = NOW() WHERE id = " + id + ";";
    // 			stringBuffer.append(sql);
    // 		}else{
    // 			String sql = "\nUPDATE tarifas_api SET pieceother=pieceother-" + amtadd  + " , updater = '"+usercode+"' , updatetime = NOW() WHERE id = " + id + ";";
    // 			stringBuffer.append(sql);
    ...
    // 	for (String id : ids) {
    // 		if("+".equals(amttype)){
    // 			String sql = "\nUPDATE tarifas_api SET baseother=baseother+" + amtadd  + " , updater = '"+usercode+"' , updatetime = NOW() WHERE id = " + id + ";";
    // 			stringBuffer.append(sql);
    // 		}else{
    // 			String sql = "\nUPDATE tarifas_api SET baseother=baseother-" + amtadd  + " , updater = '"+usercode+"' , updatetime = NOW() WHERE id = " + id + ";";
    // 			stringBuffer.append(sql);
    ...
    // 	StringBuffer stringBuffer = new StringBuffer();
    // 	for (String id : ids) {
    // 		String sql = "\nUPDATE tarifas_api SET pieceother=" + amtadd  + " , updater = '"+usercode+"' , updatetime = NOW() WHERE id = " + id + ";";
    // 		stringBuffer.append(sql);
    // 	}
    // 	//AppUtils.debug(stringBuffer);
    // 	tarifasapiDao.executeSQL(stringBuffer.toString());
    ...
    // 	StringBuffer stringBuffer = new StringBuffer();
    // 	for (String id : ids) {
    // 		String sql = "\nUPDATE tarifas_api SET baseother=" + amtadd  + " , updater = '"+usercode+"' , updatetime = NOW() WHERE id = " + id + ";";
    // 		stringBuffer.append(sql);
    // 	}
    // 	//AppUtils.debug(stringBuffer);
    // 	tarifasapiDao.executeSQL(stringBuffer.toString());
    ...
    public void updateBatchEditGrid(Object modifiedData) throws ParseException {
        JSONArray jsonArray = (JSONArray) modifiedData;
        TarifasApi ddata = new TarifasApi();
        for (int i = 0; i < jsonArray.size(); i++) {
            JSONObject jsonObject = (JSONObject) jsonArray.get(i);
            String id = String.valueOf(jsonObject.get("id"));
            String carrier = String.valueOf(jsonObject.get("carrier"));
            ...
            String effectivestartdate = String.valueOf(jsonObject.get("effectivestartdate") == null ? "" : jsonObject.get("effectivestartdate"));
            String effectiveenddate = String.valueOf(jsonObject.get("effectiveenddate") == null ? "" : jsonObject.get("typeend"));
            String condition = String.valueOf(jsonObject.get("condition") == null ? "" : jsonObject.get("condition"));

            ddata = tarifasapiDao.findById(Long.valueOf(id));
            ddata.setCarrier(carrier);
            ddata.setPol(pol);
            ddata.setPod(pod);
            ddata.setCurrency(currency);
            if (!StrUtils.isNull(effectivestartdate) && !"null".equals(effectivestartdate)) {
                ddata.setEffectivestartdate(new SimpleDateFormat("yyyy-MM-dd").parse(effectivestartdate));
            }
            if (!StrUtils.isNull(effectiveenddate) && !"null".equals(effectiveenddate)) {
                ddata.setEffectiveenddate(new SimpleDateFormat("yyyy-MM-dd").parse(effectiveenddate));
            }
            ddata.setCondition(condition);
            String rate20dv = null;
            if (jsonObject.get("rate20dv") != null) {
                rate20dv = String.valueOf(jsonObject.get("rate20dv"));
                ddata.setRate20dv(rate20dv);
            }
            String rate40dv = null;
            if (jsonObject.get("rate40dv") != null) {
                rate40dv = String.valueOf(jsonObject.get("rate40dv"));
                ddata.setRate40dv(rate40dv);
            }
            String rate40hc = null;
            if (jsonObject.get("rate40hc") != null) {
                rate40hc = String.valueOf(jsonObject.get("rate40hc"));
                ddata.setRate40hc(rate40hc);
            }
            String rate40nor = null;
            if (jsonObject.get("rate40nor") != null) {
                rate40nor = String.valueOf(jsonObject.get("rate40nor"));
                ddata.setRate40nor(rate40nor);
            }
            saveData(ddata);
        }
    }
    ...
    // 	StringBuffer stringBuffer = new StringBuffer();
    // 	for (String id : ids) {
    // 		String sql = "\nUPDATE tarifas_api SET " + column + " = '" + StrUtils.getSqlFormat(value.trim()) + "' , updater = '"+usercode+"' , updatetime = NOW() WHERE id = " + id + ";";
    // 		stringBuffer.append(sql);
    // 	}
    // 	//AppUtils.debug(stringBuffer);
    // 	tarifasapiDao.executeSQL(stringBuffer.toString());
    ...
    // 	StringBuffer stringBuffer = new StringBuffer();
    // 	for (String id : ids) {
    // 		String sql = "\nUPDATE tarifas_api SET " + column + " = COALESCE(" + column + ",'')||'" + StrUtils.getSqlFormat(value.trim()) + "' , updater = '"+usercode+"' , updatetime = NOW() WHERE id = " + id +
    // 		";";
    // 		stringBuffer.append(sql);
    // 	}
    // 	tarifasapiDao.executeSQL(stringBuffer.toString());
    ...
    public void removePriceByPriceName(String pricename) {
        String sql = "UPDATE tarifas_api SET isdelete = TRUE , updater = '" + AppUtils.getUserSession().getUsercode() + "', updatetime = NOW() WHERE pricename = '" + pricename + "' AND isdelete = FALSE";
        tarifasapiDao.executeSQL(sql);
    }
    ...
    // 	StringBuffer stringBuffer = new StringBuffer();
    // 	for (String id : ids) {
    // 		String sql = "\nUPDATE tarifas_api SET isrelease = FALSE  WHERE id = " + id + ";";
    // 		stringBuffer.append(sql);
    // 	}
    // 	tarifasapiDao.executeSQL(stringBuffer.toString());
    // }
    ...
    // 	StringBuffer stringBuffer = new StringBuffer();
    // 	for (String id : ids) {
    // 		String sql = "\nUPDATE tarifas_api SET istop = "+stop+"  WHERE id = " + id + ";";
    // 		stringBuffer.append(sql);
    // 	}
    // 	tarifasapiDao.executeSQL(stringBuffer.toString());
    // }
    ...
    // 	TarifasApi ddata = new TarifasApi();
    // 	for (String id : ids) {
    // 		TarifasApi old = tarifasapiDao.findById(Long.valueOf(id));
    //
    // 		ddata.setId(-1L);
    ...
    // 	StringBuffer stringBuffer = new StringBuffer();
    // 	for (String id : ids) {
    // 		String sql = "\nUPDATE tarifas_api SET " + column + " = null , updater = '"+usercode+"' , updatetime = NOW() WHERE id = " + id + ";";
    // 		stringBuffer.append(sql);
    // 	}
    // 	tarifasapiDao.executeSQL(stringBuffer.toString());
    // }
    ...
    // 	StringBuffer stringBuffer = new StringBuffer();
    // 	for (String id : ids) {
    // 		String sql = "\nUPDATE tarifas_api SET cost20 = null,cost40gp = null,cost40hq = null,cost45hq = null,pieceother = null, updater = '"+usercode+"' , updatetime = NOW() WHERE id = " + id + ";";
    // 		stringBuffer.append(sql);
    // 	}
    // 	tarifasapiDao.executeSQL(stringBuffer.toString());
    // }
    ...
    // 	StringBuffer stringBuffer = new StringBuffer();
    // 	for (String id : ids) {
    // 		String sql = "\nUPDATE tarifas_api SET base20 = null,base40gp = null,base40hq = null,base45hq = null,baseother = null, updater = '"+usercode+"' , updatetime = NOW() WHERE id = " + id + ";";
    // 		stringBuffer.append(sql);
    // 	}
    // 	tarifasapiDao.executeSQL(stringBuffer.toString());
    // }
    ...
    // 	StringBuffer stringBuffer = new StringBuffer();
    // 	for (String id : ids) {
    // 		String sql = "\nUPDATE tarifas_api SET cntypeothercode = '"+cntypeothercode+"', updater = '"+AppUtils.getUserSession().getUsercode()+"' , updatetime = NOW() WHERE id = " + id + ";";
    // 		stringBuffer.append(sql);
    // 	}
    // 	tarifasapiDao.executeSQL(stringBuffer.toString());
    // }
