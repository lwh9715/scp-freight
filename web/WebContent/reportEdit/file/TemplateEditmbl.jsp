			  ...
			  function(data){
			  	...
			  	if(data=="''"||data==null){
			  		var template=new TemplateEdit({
						jdatasource:{ "json": { "ORG": [ "cnortitlembl","cneetitlembl","notifytitlembl","agentitlembl","poa","pol","pdd","pod" ,"mblpol","mblpod","mblpdd","mbldestination","vessel","voyage","vesselvoyage","pretrans","destination","plable","mblsono","mblno","cntinfos","goodsinfo","carryitem","loaditem","piece","packer","markhead","cntdesc","grswgt","cbm","atdtitle","atd","totledesc","freightitemdesc","billcount","signplace","agentitle","etd","eta","carrierfullname","carriername","carrierinfo","dono","piecepacker","remark","dotype","paymentitem","bkagent","bkagentnamec","bkagentcontact","marksnombl","goodsdescmbl","goodsnamee","goodsnamee2","freightitempp","freightitemcc","mbltype","payplace","payplacepp","payplacecc","freightcharge","rate","prepaid","collect","usernamee","tel1","tel2","email1","email2","fax","jobno","freightitemmbl","remark_booking","paymentitempp","paymentitemcc","mblnombl","ldtype","sealno","cntno","vgm","now","连接符","wmsinnos","polnamecc","podnamecc","poanamecc","pddnamecc","ldtype","cnotainerinfo","hscode","userqq","jobno","cnorname","cneename","notifyname","jcustomerc","jcustomere","mbltype","sidate","bargeetd","提单要求","委托要求","备注1","备注2","第二通知人","cnotainerinfo","jobno"]}},
						jTemplates:[]
					});
			  	}else{
				    var template=new TemplateEdit({
...
<form action="./printmbl.jsp" method="post" id="from">
...
<script language="JavaScript">
	...
	function saveTemplete(json){
		//console.log("success");
		$.post("/scp/service?src=rogtemplete&action=saveRogtempletembl&userid="+<%=userid%>+"&rp="+"<%=rp%>"+"&reporttype="+"<%=reporttype%>", { "json": json },
		function(result){
			alert(result);
			//if(result.toUpperCase() == "SUCCESS"){
				location.href = location.origin + '/scp/reportEdit/file/printmbl.jsp' + location.search;
			//}
		}
		);
	}
	function printTemplete(json){
		var href = './printmbl.jsp' + location.search;
	...
	function saveTemplateForPrivate(json,text,ispublic){
		...
		$.post("/scp/service?src=rogtemplete&action=checktempletenamembl&reporttype=<%=reporttype%>&rp="+"<%=rp%>"+url+"&ispublic="+ispublic, { "json": json,"text":text },
		function(result){
			var obj = JSON.parse(result);
			if(obj.type=="success"){
				var surl = location.search;
				surl = "?rp="+ obj.value + ".raq"  + surl.substring(surl.indexOf(".raq&")+4)
				location.href = location.origin + '/scp/reportEdit/file/printmbl.jsp' + surl;
			}else{
				alert(obj.value);
			}
		});
