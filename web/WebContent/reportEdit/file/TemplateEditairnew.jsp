...
<form action="./printairnew.jsp" method="post" id="from">
...
<script language="JavaScript">
	...
	map['volwgt4']='提单体积重';
	map['volume4']='提单体积';
	map['chargeweight4']='提单计费重';
	
	map['airlinenamee']='航空公司英文';
	map['airlineno']='航空公司结算号';
	map['airlinecode']='航空公司结算代码';
	map['bookinginfo']='订舱附加信息';
	map['handinfo']='HANDLING INFO';
	map['transitport1']='中转港1';
	map['transitport2']='中转港2';
	map['flightno4']='制单航班';
	map['flightdate4']='制单航班日期';
	map['doccode']='文件操作代码';
	map['chargeweight2']='机场计费重';
	map['carriercode2']='航空代码2';
	map['bookcode']='航司订舱代码';
	map['sizeremarks']='尺寸备注';
	...
	function saveTemplete(json){
		...
		function(result){
			...
			//if(result.toUpperCase() == "SUCCESS"){
				//location.href = location.origin + '/scp/reportEdit/file/printairnew.jsp' + location.search;
			//}
			if("<%=datain%>" == "datain"){
			}else{
				location.href = location.origin + '/scp/reportEdit/file/printairnew.jsp' + location.search;
	...
	function printTemplete(json){
		var href = './printairnew.jsp' + location.search;
	...
	function saveTemplateForPrivate(json,text,ispublic){
		...
		function(result){
			...
			if(obj.type=="success"){
				...
				location.href = location.origin + '/scp/reportEdit/file/printairnew.jsp' + surl;
			}
			}else{
				alert(obj.value);
			}
