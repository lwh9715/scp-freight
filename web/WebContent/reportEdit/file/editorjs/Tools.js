...
PopupTable.prototype.CreatePopuup=function(){
	...
	div1.setAttribute("style","width:700px;");
...
function uploadimg(file){//https://www.cnblogs.com/tianma3798/p/5845291.html
	//获取文件对象
    ...
    function uploadFile(result, startIndex, onSuccess) {
        ...
        xhr.open('post', '/scp/service?src=webServer&action=templateUploads&filename='+file.name, true);
...
PopupButton.prototype.CreatePopuup=function(){
	...
	confirm.setAttribute("style","padding: 10px;font-size:12px;");
	...
	input.setAttribute("style","height: 200px;width:350px");
	input.setAttribute("id","ff-bug-input"+this.id);
	
	//var span2 = document.createElement('span');
	//span2.innerHTML="是否公开&nbsp;&nbsp;";
	
	//var checkbox=document.createElement('input');
	//checkbox.setAttribute("type","checkbox");
	//checkbox.setAttribute("title","勾选保存后其他人也能使用此模版,不勾选只有当前帐号才可使用。");
	//checkbox.setAttribute("id","ff-bug-checkbox"+this.id);
...
PopupButton.prototype.ConfirmClick=function(e)
{
	...
	if(text == undefined){
		text=$("textarea[id='ff-bug-input"+this.id+"']").val(); 
	}
...
//另存为dialog
function PopupButton2(id,Config)
{
	var Property = {id:id,text:"ss",PopupTitle:"",PopupSpan:""};
	for(p in Property)
	this[p]=Config==null||(typeof Config[p]=='undefined')?Property[p]:Config[p]
...
PopupButton2.prototype.CreatePopuup=function(){
	var obj=this;
	var confirm=document.createElement('button');
	confirm.setAttribute("class","btn btn-secondary btn-lg");
...
PopupButton2.prototype.Init=function(){
	this.CreatePopuup();
	var li =document.createElement('li');
	var a = document.createElement('a');
			var obj=this;
...
PopupButton2.prototype.ConfirmClick=function(e)
{
	var text=$("input[id='ff-bug-input"+this.id+"']").val(); 
	var checkbox=$("input[id='ff-bug-checkbox"+this.id+"']").attr('checked'); 
	this.eventConfirm2(text,checkbox);
}
...
{
	var obj=this;
	var tool1=new PopupButton('tool1',{text:"",PopupTitle:"新建模板",PopupSpan:"名称"});
	...
	var too20=new Button('too20',{text:"清除底图"});
	...
	var tool13=new PopupButton2('tool13',{text:"另存个人",PopupTitle:"新建模板",PopupSpan:"名称"});
	var tool12=new Button('tool12',{text:"临时打印"});
	...
	too20.eventClick=function(e)
	{
		obj.eventClearBackGruodImg(e);
	};
	...
	tool13.eventConfirm=function(e)
	{
		obj.DropDownTemplate.Add(e);
		obj.eventNewTemplate(e);
	};
	tool13.eventClick=function(e)
	{
		//console.log('eventClick'+e);
	};
	tool13.eventConfirm2=function(text,checkbox)
	{
		obj.eventSaveTemplateForPrivate(text,checkbox);
	};
	tool12.eventClick=function(e)
	{
		obj.eventPrintTemplate(e);
	};
	this.Add(tool1);
	this.Add(tool2);
	this.Add(tool11);
...
//		var code = search.substring(search.indexOf('&c=')+3);
//		if(code!=null&&code.length>50){
//			var decode = base64decode(code);
