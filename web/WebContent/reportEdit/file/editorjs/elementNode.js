...
ElementNode.prototype.CreateDiv = function(){ 
		...
		if(strTmp == undefined || strTmp == null){
			strTmp = '';
		}
		//console.log('strTmp:'+strTmp);
		//if(typeof(strTmp)=='string'){
		//	strTmp = strTmp.replace(/<\/br>/g,'\n');
		//	console.log('strTmp:'+strTmp);
		//	strTmp = strTmp.replace(/\n/g,"<br>");
		//	strTmp = strTmp.replace(new RegExp(/( )/g),"&nbsp;");
		//	console.log('strTmp:'+strTmp);
		//}
		//if(targetId == null){//非数据库字段，文本或文本框，保留html回车换行等
		//}

		element.innerHTML = strTmp;
...
ElementNode.prototype.CreateDivPrint=function(o){
		...
		if(flag)
		{
			targetId = this.text.replace(/^#.+\.|#$/img,'');
		...
		if("cntdesc" == columnname){
			//element.innerHTML = this.text !=null ? this.text : '';
			element.innerHTML = strTmp;
		}else{
			//console.log('strTmp:'+strTmp);
			if(typeof(strTmp)=='string'){
		...
		element.style.fontFamily="Arial,Times New Roman,宋体";
		
		//$(".weaizhi").css("letter-spacing","0.3px");
		//$(".weaizhi").css("line-height","15px");
		if(targetId){
			//20170706添加自定义属性标识字段bruce
