...
function Victor(container, anitOut) {
    if ( !! document.createElement("canvas").getContext) {
        ...
        var G = {
            ...
            xyScalar: 1,
            zOffset: 100,
            ambient: "#FFFFFF",//调整主色调 #B0E1FF  #FFFFFF
            diffuse: "#FFFFFF",
            speed: 0.001,
            gravity: 1200,
            dampening: 0.95,
...
function changeLang(){
	...
	var lang = $("input[name$='multiLanguageCom_value']").attr("value");
	$("#usernameLable").html((lang == "en")?lsObjEn.usernameLable:lsObjCn.usernameLable);
	$("#passwordLable").html((lang == "en")?lsObjEn.passwordLable:lsObjCn.passwordLable);
	$("#languageLabel").html((lang == "en")?lsObjEn.languageLabel:lsObjCn.languageLabel);
	$("#codeLabel").html((lang == "en")?lsObjEn.codeLabel:lsObjCn.codeLabel);
	$("#remeLabel").html((lang == "en")?lsObjEn.remeLabel:lsObjCn.remeLabel);
	$("#remoteLabel").html((lang == "en")?lsObjEn.remoteLabel:lsObjCn.remoteLabel);
	
	$("#versionLabel").html((lang == "en")?lsObjEn.versionLabel:lsObjCn.versionLabel);
	$("#serviceLabel").html((lang == "en")?lsObjEn.serviceLabel:lsObjCn.serviceLabel);
	$("#dateLabel").html((lang == "en")?lsObjEn.dateLabel:lsObjCn.dateLabel);
	$("#targe1").html((lang == "en")?lsObjEn.targe1:lsObjCn.targe1);
	$("#targe2").html((lang == "en")?lsObjEn.targe2:lsObjCn.targe2);
	$("#table1").html((lang == "en")?lsObjEn.table1:lsObjCn.table1);
	$("#table2").html((lang == "en")?lsObjEn.table2:lsObjCn.table2);
	$("#table3").html((lang == "en")?lsObjEn.table3:lsObjCn.table3);
	$("#table4").html((lang == "en")?lsObjEn.table4:lsObjCn.table4);
	$("#pwsLabel").html((lang == "en")?lsObjEn.pwsLabel:lsObjCn.pwsLabel);
	$("#usernameEmptyText").html((lang == "en")?lsObjEn.usernameEmptyText:lsObjCn.usernameEmptyText);
}

function setWindowIndex(){
	$('#useronlineWindow').css("z-index","99999");
	$('#upgWindow').css("z-index","99999");
	$('#resetPwdWindow').css("z-index","99999");
	
}

function init(){
	if(tipsValueJs.getValue()!=''){
		tipsWindowJsVar.show();
	}
	changerecode();
	$('button[type="submit"]').css({"height":"30px","width":"110px","font-size":"14px"});
	$("input[id*='loginForm']").css({"height":"25px"});
	$("input[id*='loginForm:multiLanguageCom']").css({"height":"20px"});
	$(".x-form-field-wrap .x-form-trigger").css({"height":"23px"});
	$("select").css({"height":"30px","width":"130px","font-size":"14px"});
	
	$(".line").slideUp();	
	if( $("#logo").attr("src") == ''){
		if("#{constantBean.headTitle}".indexOf( "UFMS")>-1){
			$("#logo").attr("src","images/logo-ufms.png");
		}else{
			$("#logo").attr("src","images/logo.png");
		}
	}
	
	var lgheight = "#{login_new.loginBean.cfgDataMap['sys_logo_height']}";
	if(parseInt(lgheight) > 0){
		$("#logo").attr("height",lgheight);
	}
	
	var h = $("#logo").attr("height");
	var ww = $("#logo").attr("width")+25+"px";
	var hh = -(h+32)*0.5+"px";
	document.getElementById('companyName').style.cssText="display:block; margin-top: "+hh+";margin-left:"+ww+";font-size: 28px;font-family:'楷体'";
	
	
	//var beianHtml = '#{login_new.loginBean.cfgDataMap["sys_ICP_NO"]}';
	//$("div[class='banner']").append(beianHtml);
	
	
	
	
	$("input[id*='username']").blur(function(){
		var ua = $("input[id*='username']").val();
		ua.replace(/\s+/g,"");
	});
}


function changerecode(){
	if($("#recode").html()==''){
	$("#recorddiv").hide();
	}
}