...
function autoCalcAmountwfremaind() {
	...
	store.each(function(r) {
		...
		rpsumArray[currencyto].amtrest = rpsumArray[currencyto].amt + rpsumArray[currencyto].amtother - Math.abs((rpsumArray[currencyto].ar - rpsumArray[currencyto].ap));
	...
	store.each(function(r) {
		...
		if(r.get("amountwf") >0){
			if(agentcorpidJsVar.getValue() == false){//未勾选代收代付
				if(arapcorpid != accountCorpJsVar.getValue()){
